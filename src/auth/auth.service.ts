...
type IDoneReg = IDone<IUser>;

class AuthService {
  async getOtp({ mobile, email }: IGetOtp, done: IDone<string>) {
    try {
      if (!mobile || !email) return done(new Error("mobile email is required"));
  ...
  async register({ mobile, email, otp, hash }: IRegister, done: IDoneReg) {
    try {
  ...
  async verifyUser(
    email: string,
    mobile: string,
    done: (err: any, user?: IUser | false, info?: { message: string }) => void
  ) {}
